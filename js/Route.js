...
var Type, asyncTry, coalesce, createRegex, emptyArray, isValid, matchAll, matchRegex, readBody, type, validateQuery, validateTypes, valido;
...
type.defineMethods({
  ...
  _build: function(responder, method) {
    ...
    return function(req, pathname) {
      ...
      if (matchRegex(regex, pathname, req.query)) {
        return route;
      }
    };
  },
...
matchRegex = function(regex, pathname, query) {
  var match;
  regex.lastIndex = 0;
  ...
  match.slice(1).forEach(function(value, index) {
    query[regex.params[index] || index] = value;
  });
  return true;
};
